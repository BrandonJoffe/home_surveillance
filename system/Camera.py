...
class IPCamera(object):
	...
	def dump_video_info(self):
		...
		logger.info("The U value of the whitebalance setting (note: only supported by DC1394 v 2.x backend currently)")
		logger.info(self.video.get(cv.CV_CAP_PROP_WHITE_BALANCE_U))
		logger.info("The V value of the whitebalance setting (note: only supported by DC1394 v 2.x backend currently)")
		logger.info(self.video.get(cv.CV_CAP_PROP_WHITE_BALANCE_V))
		logger.info("Rectification flag for stereo cameras (note: only supported by DC1394 v 2.x backend currently)")
		logger.info(self.video.get(cv.CV_CAP_PROP_RECTIFICATION))
		logger.info("The ISO speed of the camera (note: only supported by DC1394 v 2.x backend currently)")
		logger.info(self.video.get(cv.CV_CAP_PROP_ISO_SPEED))
		logger.info("Amount of frames stored in internal buffer memory (note: only supported by DC1394 v 2.x backend currently)")
		logger.info(self.video.get(cv.CV_CAP_PROP_BUFFERSIZE))
